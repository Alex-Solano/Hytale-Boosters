...
export function openBooster(count, boosterId, odds, allowNames = null) {
  ...
  if (basePool.length === 0) {
    console.warn("WARN: Aucune carte trouv�e pour booster:", boosterId);
    return [];
  }
  ...
  if (Array.isArray(allowNames) && allowNames.length > 0) {
    ...
    if (filtered.length === 0) {
      console.warn("WARN: Pool filtr� vide, fallback sur le pool complet pour:", boosterId);
    } else {
      pool = filtered;
    }
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    // s�curit� : si pas de cartes de cette raret�, on redescend
    let tries = 0;
